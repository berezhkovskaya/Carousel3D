...
void main()
{
   gl_Position = in_ModelViewProjectionMatrix * gl_Vertex;
...
void main()
{
   ...
   for ( int i = 0 ; i != 3 ; i++ )
   {
	...
	vec4 I = vec4(1.0)-sqrt( Rx * Rx + Ry * Ry );
	float InvBrightness = dot( I, vec4( 0.3, 0.59, 0.11, 0.5 ) );

   out_FragColor = vec4(vec3(InvBrightness),1.0);
}
