...
void main()
{
   ...
   Position += X * Coord.x * SizeX;
   Position += Y * Coord.y * SizeY;
...
void main()
{
   ...
   float LightUpPoint = 0.1;
 
   float Phase = 1.0 - (TTL / LifeTime);

   if ( Phase < LightUpPoint )
   {
      Phase = Phase / LightUpPoint;
   }
   else
   {
      Phase = 1.0 - ( Phase - LightUpPoint ) / ( 1.0 - LightUpPoint );
   }

   vec4 Color = TexCoord2 * ParticleColor * Phase; 
