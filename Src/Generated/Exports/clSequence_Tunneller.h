/**
 * \file clSequence_Tunneller.h
 * \brief Tunneller for class: clSequence
 * \version 0.6.06
 * \date 16/01/2012
...
//                             Base class: clSelector  (public)

#ifndef _clSequence_Tunneller_
#define _clSequence_Tunneller_

#include "Linderdaum/World/AI/Behaviour.h"

#if !defined( _DISABLE_TUNNELLERS_ )

class clSequence_Tunneller: public clSequence
{
public:
   //
   // iObject interface
   ...
   virtual void    DumpDot(iOStream* P0)
   {
      if ( !iObject::FInheritedCall && iObject::IsMethodOverriden( "DumpDot" ) )
      {
         ...
         if ( !MethodCalled ) clSequence::DumpDot(P0);

         return;
      }

      clSequence::DumpDot(P0);
   }
   ...
   //
   // clSelector interface
   //
   virtual void    Restart()
   {
      if ( !iObject::FInheritedCall && iObject::IsMethodOverriden( "Restart" ) )
      {
         clParametersList Params;

         bool MethodCalled = iObject::CallMethod( "Restart", Params, iObject::FInheritedCall );

         if ( !MethodCalled ) clSelector::Restart();
   ...
   {
      if ( !iObject::FInheritedCall && iObject::IsMethodOverriden( "SelectNext" ) )
      {
         clParametersList Params;

         bool MethodCalled = iObject::CallMethod( "SelectNext", Params, iObject::FInheritedCall );

         if ( !MethodCalled ) return clSequence::SelectNext();

         return *(TypeTraits< int >::ReferredType*)iObject::GetReturnValue()->GetNativeBlock();
      }

      return clSequence::SelectNext();
   }
   virtual int    GetCurrentBehaviour() const
   {
      if ( !iObject::FInheritedCall && iObject::IsMethodOverriden( "GetCurrentBehaviour" ) )
      {
         clParametersList Params;

         bool MethodCalled = iObject::CallMethod( "GetCurrentBehaviour", Params, iObject::FInheritedCall );

         if ( !MethodCalled ) return clSelector::GetCurrentBehaviour();

         return *(TypeTraits< int >::ReferredType*)iObject::GetReturnValue()->GetNativeBlock();
      }

      return clSelector::GetCurrentBehaviour();
   }
   virtual void    SetCurrentBehaviour(int P0)
   {
      if ( !iObject::FInheritedCall && iObject::IsMethodOverriden( "SetCurrentBehaviour" ) )
      {
         clParametersList Params;
         ...
         bool MethodCalled = iObject::CallMethod( "SetCurrentBehaviour", Params, iObject::FInheritedCall );

         if ( !MethodCalled ) clSelector::SetCurrentBehaviour(P0);

         return;
      }

      clSelector::SetCurrentBehaviour(P0);
   }
   //
   // clSequence interface
   //
};
