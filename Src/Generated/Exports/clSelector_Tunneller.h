/**
 * \file clSelector_Tunneller.h
 * \brief Tunneller for class: clSelector
 * \version 0.6.06
 * \date 16/01/2012
...
//                             Base class: clCompositeBehaviour  (public)

#ifndef _clSelector_Tunneller_
#define _clSelector_Tunneller_

#include "Linderdaum/World/AI/Behaviour.h"

#if !defined( _DISABLE_TUNNELLERS_ )

class clSelector_Tunneller: public clSelector
{
public:
   //
   // iObject interface
   ...
   virtual void    DumpDot(iOStream* P0)
   {
      if ( !iObject::FInheritedCall && iObject::IsMethodOverriden( "DumpDot" ) )
      {
         ...
         if ( !MethodCalled ) clSelector::DumpDot(P0);

         return;
      }

      clSelector::DumpDot(P0);
   }
   ...
   virtual int    SelectNext()
   {
      if ( !iObject::FInheritedCall && iObject::IsMethodOverriden( "SelectNext" ) )
      {
         ...
         if ( !MethodCalled ) return clSelector::SelectNext();

         return *(TypeTraits< int >::ReferredType*)iObject::GetReturnValue()->GetNativeBlock();
      }

      return clSelector::SelectNext();
   }
